...
fields) cannot be passed to the `~gaussdb.Cursor.execute()` method like query
arguments::
...
name should be escaped using `~gaussdb.pq.Escaping.escape_identifier()`::
    ...
    from gaussdb.pq import Escaping
...
The objects exposed by the `!gaussdb.sql` module allow generating SQL
...
`~gaussdb.Cursor.execute()`: such value placeholders will be untouched by
`!format()`::
...
`~gaussdb.Cursor.copy()`, but can also be used to compose a query as a Python
