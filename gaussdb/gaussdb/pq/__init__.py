...
__impl__: str
"""The currently loaded implementation of the `!gaussdb.pq` package.
...
A number in the same format of `~gaussdb.ConnectionInfo.server_version`
...
def import_from_libpq() -> None:
    ...
    impl = os.environ.get("GAUSSDB_IMPL", "").lower()
    module = None
    attempts: list[str] = []

    def handle_error(name: str, e: Exception) -> None:
        if not impl:
            msg = f"couldn't import gaussdb '{name}' implementation: {e}"
            attempts.append(msg)
        else:
            msg = f"couldn't import requested gaussdb '{name}' implementation: {e}"
    ...
    if module:
        ...
        raise ImportError(f"requested gaussdb implementation '{impl}' unknown")
    else:
