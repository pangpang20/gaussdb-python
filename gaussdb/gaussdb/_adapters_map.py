...
class AdaptersMap:
    ...
    `~gaussdb.abc.AdaptContext.adapters` attribute: changing such map allows
    ...
    `~gaussdb.Cursor` is created from a `~gaussdb.Connection`), the parent's
    ...
    template, exposed as `gaussdb.adapters`: changing such mapping allows to
    ...
    def get_dumper(self, cls: type, format: PyFormat) -> type[Dumper]:
        ...
        Raise `~gaussdb.ProgrammingError` if a class is not available.

        :param cls: The class to adapt.
        :param format: The format to dump to. If `~gaussdb.adapt.PyFormat.AUTO`,
    ...
    def _get_optimised(self, cls: type[RV]) -> type[RV]:
        ...
        # Check if the class comes from gaussdb.types and there is a class
        # with the same name in gaussdb_c._gaussdb.
        from gaussdb import types

        if cls.__module__.startswith(types.__name__):
            new = cast("type[RV]", getattr(_gaussdb, cls.__name__, None))
            if new:
