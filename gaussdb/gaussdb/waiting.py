...
def wait_epoll(gen: PQGen[RV], fileno: int, interval: float | None = None) -> RV:
    ...
    To reproduce the bug:
        ...
        export GAUSSDB_WAIT_FUNC=wait_epoll
...
if "GAUSSDB_WAIT_FUNC" in os.environ:
    fname = os.environ["GAUSSDB_WAIT_FUNC"]
    if not fname.startswith("wait_") or fname not in globals():
        raise ImportError(
            "GAUSSDB_WAIT_FUNC should be the name of an available wait function;"
            f" got {fname!r}"
        )
    wait = globals()[fname]
...
elif _gaussdb and sys.platform != "win32" and not _is_select_patched():
    wait = wait_c
