...
class Dumper(abc.Dumper, ABC):
    ...
    def quote(self, obj: Any) -> Buffer:
        ...
        # usually it means by gaussdb.sql.quote() or by
    ...
    def get_key(self, obj: Any, format: PyFormat) -> abc.DumperKey:
        """
        Implementation of the `~gaussdb.abc.Dumper.get_key()` member of the
    ...
    def upgrade(self, obj: Any, format: PyFormat) -> Dumper:
        """
        Implementation of the `~gaussdb.abc.Dumper.upgrade()` member of the
        `~gaussdb.abc.Dumper` protocol. Look at its definition for details.
