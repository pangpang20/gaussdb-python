...
class AdaptContext(Protocol):
    ...
    Example of `~AdaptContext` are `~gaussdb.Connection`, `~gaussdb.Cursor`,
    `~gaussdb.adapt.Transformer`, `~gaussdb.adapt.AdaptersMap`.
    ...
    def connection(self) -> BaseConnection[Any] | None:
        ...
        :rtype: `~gaussdb.Connection` or `~gaussdb.AsyncConnection` or `!None`
        """
        ...
...
class Dumper(Protocol):
    ...
    `~gaussdb.pq.Format.TEXT` or `~gaussdb.pq.Format.BINARY`.
