...
class Composable(ABC):
    ...
    `~gaussdb.Cursor.execute()`, `~gaussdb.Cursor.executemany()`,
    `~gaussdb.Cursor.copy()` in place of the query string.
    """
    ...
    def as_bytes(self, context: AdaptContext | None = None) -> bytes:
        ...
        The method is automatically invoked by `~gaussdb.Cursor.execute()`,
        `~gaussdb.Cursor.executemany()`, `~gaussdb.Cursor.copy()` if a
...
class Composed(Composable):
    ...
    directly to `~gaussdb.Cursor.execute()`, `~gaussdb.Cursor.executemany()`,
    `~gaussdb.Cursor.copy()` in place of the query string.
...
class SQL(Composable):
    ...
    `!SQL` objects can be passed directly to `~gaussdb.Cursor.execute()`,
    `~gaussdb.Cursor.executemany()`, `~gaussdb.Cursor.copy()` in place of the
    query string.
