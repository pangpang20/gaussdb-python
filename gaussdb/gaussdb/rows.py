...
class RowFactory(Protocol[Row]):
    """
    Callable protocol taking a `~gaussdb.Cursor` and returning a `RowMaker`.
    ...
    `~gaussdb.Cursor.description` attribute) and help a `!RowMaker` to create
    a complete object.
...
def tuple_row(cursor: BaseCursor[Any, Any]) -> RowMaker[TupleRow]:
    ...
    This is the default factory, used when `~gaussdb.Connection.connect()` or
    `~gaussdb.Connection.cursor()` are called without a `!row_factory`
    parameter.
