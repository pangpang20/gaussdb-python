...
class FinishedPGconn:
    ...
    Attributes are set from a real `~gaussdb.pq.PGconn` but any operations will
    raise an `~gaussdb.OperationalError`.
    """
...
class Warning(Exception):
    ...
    Defined for DBAPI compatibility, but never raised by ``gaussdb``.
    """
...
class Error(Exception):
    ...
    def pgresult(self) -> PGresult | None:
        ...
        :rtype: gaussdb.pq.PGresult | None
        """
...
class ConnectionTimeout(OperationalError):
    """
    Exception raised on timeout of the `~gaussdb.Connection.connect()` method.
...
class CancellationTimeout(OperationalError):
    ...
    `~gaussdb.Connection.cancel_safe()` method.
