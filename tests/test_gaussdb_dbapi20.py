...
class PsycopgTPCTests(dbapi20_tpc.TwoPhaseCommitTests):
    ...
    def connect(self):
        return gaussdb.connect(*self.connect_args)
...
def test_singletons(conn, typename, singleton):
    singleton = getattr(gaussdb, singleton)
    cur = conn.cursor()
...
def test_timestamp_from_ticks(ticks, want):
    s = gaussdb.TimestampFromTicks(ticks)
